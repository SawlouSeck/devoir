...
{$programme = Programme::find(1);
    ...
    public function vote(Programme $programme)
    {
        // Exemple de programme avec l'id 1, ajustez cela selon votre besoin
        $programme = Programme::find(1);

        // Obtenez le nombre de likes pour ce programme
        $likesCount = $programme->likes()->where('type', 'like')->count();

        // Obtenez le nombre de dislikes pour ce programme (ajoutez cela si nécessaire)
        $dislikesCount = $programme->likes()->where('type', 'dislike')->count();

        // Obtenez le nombre total de likes de l'utilisateur
        $userLikeCount = auth()->user()->likes()->count();

        return view('programmes.vote', [
            'program' => $programme,
            'likes' => $likesCount,
